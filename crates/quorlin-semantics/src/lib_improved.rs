...
impl SemanticAnalyzer {
    ...
    fn check_statement(&mut self, stmt: &quorlin_parser::Stmt) -> SemanticResult<()> {
        ...
        match stmt {
            ...
            Stmt::AugAssign(aug) => {
                // Check target exists and get its type
                if let Some(var_type) = self.symbols.lookup_variable(&aug.target) {
                    // Check that variable is initialized
                    if !self.initialized_vars.contains(&aug.target) {
                        return Err(SemanticError::UninitializedVariable(aug.target.clone()));
                    }

                    let value_type = self.check_expression(&aug.value)?;

                    // Infer operation type
                    use quorlin_parser::{AugAssignOp, BinOp};
                    let binop = match aug.op {
                    ...
                    let result_type = type_checker::infer_binop_type(var_type, &value_type, &binop)?;
                    type_checker::check_type_compatibility(var_type, &result_type)?;
                } else {
                    return Err(SemanticError::UndefinedVariable(aug.target.clone()));
                }
                Ok(())
            }
            Stmt::Revert(_msg) => Ok(()),
    ...
    fn check_expression(&mut self, expr: &quorlin_parser::Expr) -> SemanticResult<Type> {
        ...
        match expr {
            ...
            Expr::Call(func, args) => {
                ...
                // Full function type inference would require function signature lookup
                Ok(Type::Simple("unknown".to_string()))
            }
