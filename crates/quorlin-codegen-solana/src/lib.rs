    ...
    fn map_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Mapping(key, val) => {
                // Mappings in Solana are handled via BTreeMap for deterministic serialization
                format!("BTreeMap<{}, {}>", self.map_type(key), self.map_type(val))
            }
            Type::List(inner) => {
                                ...
                                if let Expr::Attribute(base, attr) = &**nested_target {
                                    if let Expr::Ident(base_name) = &**base {
                                        if base_name == "self" {
                                            ...
                                            code.push_str(&format!("{}contract.{}.entry({}).or_insert_with(BTreeMap::new).insert({}, {});\n",
