...
impl SolanaCodegen {
    ...
    pub fn generate(&mut self, module: &Module) -> CodegenResult<String> {
        ...
        code.push_str(&self.generate_accounts(&contract.body)?);

        // Generate events
        code.push_str(&self.generate_events()?);

        // Generate error codes
        code.push_str(&self.generate_error_codes()?);
    ...
    fn map_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Mapping(key, val) => {
                // Anchor IDL doesn't support Map types, use Vec of tuples instead
                format!("Vec<({}, {})>", self.map_type(key), self.map_type(val))
            }
            Type::List(inner) => {
    ...
    fn generate_initialize(&self, members: &[ContractMember]) -> CodegenResult<String> {
        ...
        if let Some(func) = init_func {
            ...
            code.push_str("        let signer = ctx.accounts.signer.key();\n\n");
        ...
        match stmt {
            ...
            Stmt::Assign(assign) => {
                match &assign.target {
                    ...
                    Expr::Index(target, index) => {
                        // Check if value references the same index (to avoid borrow checker issues)
                        let needs_temp = self.value_references_target(&assign.value, target, index);

                        if needs_temp {
                            // Generate temp variable first to avoid borrow checker issues
                            let temp_var = format!("temp_value_{}", indent);
                            let value_code = self.generate_expression(&assign.value)?;
                            code.push_str(&format!("{}let {} = {};\n", indent_str, temp_var, value_code));

                            // Now do the assignment with the temp var
                            if let Expr::Index(nested_target, nested_index) = &**target {
                                if let Expr::Attribute(base, attr) = &**nested_target {
                                    if let Expr::Ident(base_name) = &**base {
                                        if base_name == "self" {
                                            ...
                                            // Nested Vec insert operation
                                            code.push_str(&format!("{}if let Some((_, inner)) = contract.{}.iter_mut().find(|(k, _)| k == &{}) {{\n", indent_str, attr, first_key));
                                            code.push_str(&format!("{}    if let Some(entry) = inner.iter_mut().find(|(k, _)| k == &{}) {{\n", indent_str, second_key));
                                            code.push_str(&format!("{}        entry.1 = {};\n", indent_str, temp_var));
                                            code.push_str(&format!("{}    }} else {{\n", indent_str));
                                            code.push_str(&format!("{}        inner.push(({}, {}));\n", indent_str, second_key, temp_var));
                                            code.push_str(&format!("{}    }}\n", indent_str));
                                            code.push_str(&format!("{}}} else {{\n", indent_str));
                                            code.push_str(&format!("{}    contract.{}.push(({}, vec![({}, {})]));\n", indent_str, attr, first_key, second_key, temp_var));
                                            code.push_str(&format!("{}}}\n", indent_str));
                                        }
                                    }
                                }
                                ...
                                if let Expr::Ident(base_name) = &**base {
                                    if base_name == "self" {
                                        ...
                                        // Vec insert operation
                                        code.push_str(&format!("{}if let Some(entry) = contract.{}.iter_mut().find(|(k, _)| k == &{}) {{\n", indent_str, attr, key_code));
                                        code.push_str(&format!("{}    entry.1 = {};\n", indent_str, temp_var));
                                        code.push_str(&format!("{}}} else {{\n", indent_str));
                                        code.push_str(&format!("{}    contract.{}.push(({}, {}));\n", indent_str, attr, key_code, temp_var));
                                        code.push_str(&format!("{}}}\n", indent_str));
                                    }
                                }
                            }
                                ...
                                if let Expr::Attribute(base, attr) = &**nested_target {
                                    if let Expr::Ident(base_name) = &**base {
                                        if base_name == "self" {
                                            ...
                                            code.push_str(&format!("{}    }} else {{\n", indent_str));
                                            code.push_str(&format!("{}        inner.push(({}, {}));\n", indent_str, second_key, value_code));
                                            code.push_str(&format!("{}    }}\n", indent_str));
                                            code.push_str(&format!("{}}} else {{\n", indent_str));
            ...
            Stmt::Require(req) => {
                ...
                if let Some(msg) = &req.message {
                    // Map error messages to ErrorCode variants
                    let error_code = match msg.as_str() {
                        "Insufficient balance" => "ErrorCode::InsufficientBalance",
                        "Insufficient allowance" => "ErrorCode::InsufficientAllowance",
                        "Cannot send to zero address" => "ErrorCode::ZeroAddress",
                        "Cannot approve zero address" => "ErrorCode::ZeroApproval",
                        _ => return Err(CodegenError::UnsupportedFeature(format!("Unknown error message: {}", msg))),
                    };
                    code.push_str(&format!("{}require!({}, {});\n", indent_str, cond, error_code));
                } else {
                    code.push_str(&format!("{}require!({});\n", indent_str, cond));
                }
            }
            ...
            Expr::Index(target, index) => {
                ...
                if let Expr::Index(nested_target, nested_index) = &**target {
                    ...
                    if let Expr::Attribute(base, attr) = &**nested_target {
                        if let Expr::Ident(base_name) = &**base {
                            if base_name == "self" {
                                ...
                                let second_key = self.generate_expression(index)?;
                                return Ok(format!(
                                    "contract.{}.iter().find(|(k, _)| k == &{}).and_then(|(_, inner)| inner.iter().find(|(k, _)| k == &{}).map(|(_, v)| *v)).unwrap_or_default()",
                    ...
                    if let Expr::Ident(base_name) = &**base {
                        if base_name == "self" {
                            let key_code = self.generate_expression(index)?;
                            return Ok(format!("contract.{}.iter().find(|(k, _)| k == &{}).map(|(_, v)| *v).unwrap_or_default()", attr, key_code));
                        }
                    }
                }
    ...
    fn value_references_target(&self, value: &Expr, target: &Expr, index: &Expr) -> bool {
        ...
        match value {
            Expr::Call(_, _) => true,  // Assume function calls might reference the target
            Expr::BinOp(_, _, _) => true,  // Binary operations might reference the target
            _ => false,
        }
    }

    /// Generate account structures
    fn generate_accounts(&self, members: &[ContractMember]) -> CodegenResult<String> {
        let mut code = String::new();
        ...
        // Generate individual account structures for each function
        for member in members {
            if let ContractMember::Function(func) = member {
                // Skip constructor
                if func.name == "__init__" {
                ...
                let struct_name = self.to_pascal_case(&func.name);
                code.push_str("#[derive(Accounts)]\n");
                code.push_str(&format!("pub struct {}<'info> {{\n", struct_name));

                // Check if function modifies state (has return type and is a view)
                let is_view = func.return_type.is_some() &&
                    (func.name.starts_with("get_") || func.name.starts_with("balance_of") ||
                     func.name.starts_with("allowance") || func.name.contains("_of"));
    ...
    fn generate_error_codes(&self) -> CodegenResult<String> {
        let mut code = String::new();
        ...
        code.push_str("    #[msg(\"Insufficient allowance\")]\n");
        code.push_str("    InsufficientAllowance,\n");
        code.push_str("    #[msg(\"Cannot send to zero address\")]\n");
        code.push_str("    ZeroAddress,\n");
        code.push_str("    #[msg(\"Cannot approve zero address\")]\n");
        code.push_str("    ZeroApproval,\n");
        code.push_str("}\n");
