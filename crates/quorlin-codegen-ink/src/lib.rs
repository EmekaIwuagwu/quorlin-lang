...
impl InkCodegen {
    ...
    pub fn generate(&mut self, module: &Module) -> CodegenResult<String> {
        ...
        code.push_str("    use ink::prelude::string::String;\n\n");
    ...
    fn generate_header(&self) -> String {
        ...
        // No need for external U256 import - using u128
        code
    }
    ...
    fn map_type(&self, ty: &Type) -> String {
        match ty {
            Type::Simple(s) => match s.as_str() {
                ...
                "uint256" => "u128".to_string(), // ink! v5: use u128 for simplicity (U256 lacks StorageLayout)
                "int8" => "i8".to_string(),
                "int16" => "i16".to_string(),
                "int32" => "i32".to_string(),
                "int64" => "i64".to_string(),
            ...
            Type::Mapping(key, val) => {
                ...
                if let Type::Mapping(inner_key, inner_val) = &**val {
                    // ink! doesn't support nested Mapping, use tuple key instead
                    format!("Mapping<({}, {}), {}>",
                        self.map_type(key),
                        self.map_type(inner_key),
                        self.map_type(inner_val))
                } else {
    ...
    fn generate_statement(&self, stmt: &Stmt, indent: usize, in_constructor: bool) -> CodegenResult<String> {
        ...
        match stmt {
                            ...
                            if let Expr::Attribute(base, attr) = &**nested_target {
                                if let Expr::Ident(base_name) = &**base {
                                    if base_name == "self" {
                                        ...
                                        let prefix = if in_constructor { "instance" } else { "self" };
                                        // Use tuple key for nested mapping in ink!
            ...
            Stmt::Emit(emit) => {
                // Emit event - always use Self::env() for consistency
                code.push_str(&format!("{}Self::env().emit_event({} {{\n", indent_str, emit.event));
    ...
    fn generate_expression(&self, expr: &Expr, in_constructor: bool) -> CodegenResult<String> {
        match expr {
            Expr::IntLiteral(n) => {
                // Special case: if it's 0 and used in address context, create zero AccountId
                // This is a heuristic; ideally we'd have type information here
                Ok(n.clone())
            }
            ...
            Expr::BinOp(left, op, right) => {
                ...
                let mut right_code = self.generate_expression(right, in_constructor)?;

                // Special case: comparing address to 0, use proper zero AccountId
                if matches!(op, BinOp::NotEq | BinOp::Eq) && right_code == "0" {
                    right_code = "AccountId::from([0u8; 32])".to_string();
                }

                // Use checked arithmetic for ink! safety
                match op {
                    BinOp::Add => Ok(format!("{}.checked_add({}).expect(\"arithmetic overflow\")", left_code, right_code)),
                    BinOp::Sub => Ok(format!("{}.checked_sub({}).expect(\"arithmetic underflow\")", left_code, right_code)),
                    BinOp::Mul => Ok(format!("{}.checked_mul({}).expect(\"arithmetic overflow\")", left_code, right_code)),
                    BinOp::Div => Ok(format!("{}.checked_div({}).expect(\"division by zero\")", left_code, right_code)),
                    BinOp::Mod => Ok(format!("{}.checked_rem({}).expect(\"modulo by zero\")", left_code, right_code)),
                    BinOp::Eq => Ok(format!("({} == {})", left_code, right_code)),
                    BinOp::NotEq => Ok(format!("({} != {})", left_code, right_code)),
                    BinOp::Lt => Ok(format!("({} < {})", left_code, right_code)),
                    BinOp::Gt => Ok(format!("({} > {})", left_code, right_code)),
                    BinOp::LtEq => Ok(format!("({} <= {})", left_code, right_code)),
                    BinOp::GtEq => Ok(format!("({} >= {})", left_code, right_code)),
                    BinOp::And => Ok(format!("({} && {})", left_code, right_code)),
                    BinOp::Or => Ok(format!("({} || {})", left_code, right_code)),
                    _ => Err(CodegenError::UnsupportedFeature(format!("BinOp {:?}", op))),
                }
            }
            Expr::Call(func, args) => {
                if let Expr::Ident(func_name) = &**func {
                    ...
                    match func_name.as_str() {
                        "address" => {
                            if args.len() == 1 {
                                // Special case: address(0) should become zero AccountId
                                if arg_codes[0] == "0" {
                                    Ok("AccountId::from([0u8; 32])".to_string())
                                } else {
            ...
            Expr::Attribute(base, attr) => {
                if let Expr::Ident(base_name) = &**base {
                    if base_name == "msg" && attr == "sender" {
                        // Always use Self::env() for consistency
                        return Ok("Self::env().caller()".to_string());
            ...
            Expr::Index(target, index) => {
                ...
                if let Expr::Index(nested_target, nested_index) = &**target {
                    // Nested mapping access with tuple key
                    if let Expr::Attribute(base, attr) = &**nested_target {
                        if let Expr::Ident(base_name) = &**base {
                            if base_name == "self" {
                                let first_key = self.generate_expression(nested_index, in_constructor)?;
