...
pub mod token {
    ...
    pub fn transfer_from(
        ...
        let temp_value_8 = contract.allowances.iter().find(|(k, _)| k == &from_addr).and_then(|(_, inner)| inner.iter().find(|(k, _)| k == &signer).map(|(_, v)| *v)).unwrap_or_default().checked_sub(amount).expect("arithmetic underflow");
